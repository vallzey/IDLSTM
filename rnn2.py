...
from wgate import HGate


class RNNLayer(MergeLayer):
    def __init__(self, incoming, num_units,
        ...
        super(RNNLayer, self).__init__(incomings, **kwargs)
        ...
        def add_cell_params(gate, gate_name):
        ...
        def add_hid_params(gate, gate_name):
            """ Convenience function for adding layer parameters from a Gate
            instance. """
            return (self.add_param(gate.W_c, (num_units, num_units),
                                   name="W_hid_to_{}".format(gate_name)),
                    self.add_param(gate.b, (num_units,),
                                   name="b_{}".format(gate_name),
                                   regularizable=False),
        ...
        (self.W_in_to_cell, self.W_hid_to_cell, self.b_cell,
         self.nonlinearity_cell) = add_cell_params(cell, 'cell')

        outgate = HGate()
        (self.W_cell_to_hid, self.b_hid,
         self.nonlinearity_hid) = add_hid_params(outgate, 'outgate')
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step(input_n, cell_previous, hid_previous, *args):
            ...
            gates = T.dot(input_n, self.W_in_to_cell) + T.dot(hid_previous, self.W_hid_to_cell) + self.b_cell
            ...
            cell_input = slice_w(gates, 0)

            # Apply nonlinearities
            # ingate = self.nonlinearity_ingate(ingate)
            # forgetgate = self.nonlinearity_forgetgate(forgetgate)
            cell = self.nonlinearity_cell(cell_input)
            # Compute new cell value

            # Compute new hidden unit activation
            hid = self.nonlinearity_hid(T.dot(cell, self.W_cell_to_hid) + self.b_hid)
            return [cell, hid]
        ...
        non_seqs = [self.W_in_to_cell, self.W_hid_to_cell, self.b_cell, self.W_cell_to_hid, self.b_hid]
