...
from tgate import OutGate, TimeGate, DurationGate
from wgate import WGate  # addvw
...
class VDTLSTMLayer(MergeLayer):
    def __init__(self,
        ...
        super(VDTLSTMLayer, self).__init__(incomings, **kwargs)
        ...
        def add_timegate_params(gate, gate_name):
            return (
                self.add_param(gate.W_t, (1, num_units),
        ...
        def add_duration_gate_params(gate, gate_name):
            return (
                self.add_param(gate.W_d, (1, num_units),
        ...
        # addv
        # (self.W_d1_to_dg1, self.W_x1_to_dg1, self.b1_dg1, self.nonlinearity_dg1,
        #  self.nonlinearity_outside_dg1) = add_duration_gate_params(dgate1, 'dgate1')

        (self.W_d2_to_dg2, self.W_x2_to_dg2, self.b2_dg2, self.nonlinearity_dg2,
         self.nonlinearity_outside_dg2) = add_duration_gate_params(dgate2, 'dgate2')

        # addvw 添加一个权重w
        (self.W_x_wg, self.b_wg, self.nonlinearity_wg) = add_wgate_params(wgate, 'wgate')
        ...
        if isinstance(hid1_init, Layer):
            ...
            self.hid1_init = self.add_param(
                hid1_init, (1, self.num_units), name="hid1_init",
                trainable=learn_init, regularizable=False)

        if isinstance(hid2_init, Layer):
            self.hid2_init = hid2_init
        else:
            self.hid2_init = self.add_param(
                hid2_init, (1, self.num_units), name="hid2_init",
                trainable=learn_init, regularizable=False)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        if self.precompute_input:
            ...
            # addv
            duration_input = T.dot(duration_input, W_d_stacked)
        ...
        def step(input_n, time_input_n, duration_input_n, cell_previous, hid_previous, hid1_pre, hid2_pre, n_pre,
                 *args):
            ...
            if not self.precompute_input:
                ...
                # addv
                duration_input_n = T.dot(duration_input_n, W_d_stacked)
            ...
            # dm_w1_n = slice_w(duration_input_n, 1)

            dm_w2_n = self.nonlinearity_dg2(dm_w2_n)
            # dm_w1_n = self.nonlinearity_dg1(dm_w1_n)
            ...
            dm2_xwb_n = slice_w(input_n, 5)
            # dm1_xwb_n = slice_w(input_n, 7)
            duration_gate2 = self.nonlinearity_outside_dg2(dm_w2_n + dm2_xwb_n)
            # duration_gate1 = self.nonlinearity_outside_dg1(dm_w1_n + dm1_xwb_n)
            ...
            # add2
            # addv
            cell = forgetgate * cell_previous + duration_gate2 * ingate * cell_input
            # tilde_cell = forgetgate * cell_previous + ingate * timegate1 * cell_input
            tilde_cell = ingate * timegate1 * cell_input + cell
            ...
            # addv22_6
            hid1 = self.nonlinearity(T.dot(hid, self.W_x_wg) + self.b_wg)
            n = n_pre
            hid2 = hid1
        ...
        def step_masked(input_n,
                        ...
                        mask_n, cell_previous, hid_previous, hid1_pre, hid2_pre, n_pre, *args):
            cell, hid, hid1, hid2, n = step(input_n,
                                            time_input_n,  # add 添加时间的输入
                                            duration_input_n,  # addv
                                            cell_previous, hid_previous, hid1_pre, hid2_pre, n_pre, *args)
        ...
        if not isinstance(self.hid_init, Layer):
            ...
            hid1_init = T.dot(zeros, self.hid1_init)
            hid2_init = T.dot(zeros, self.hid2_init)
